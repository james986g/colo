...
endpoint4() {
    ...
    # Put the generated IP segment list into ip.txt
    echo ${temp[@]} | sed -e 's/ /\n/g' | sort -u >ip.txt
...
# New function for one-key cleanup
cleanup() {
    green "Starting cleanup of temporary files..."
    rm -f warp ip.txt result.csv
    if [ $? -eq 0 ]; then
        green "Cleanup completed successfully!"
    else
        red "Cleanup failed, some files may not exist or there was an error."
    fi
    echo ""
}
...
menu() {
    ...
    echo -e " ${GREEN}3.${PLAIN} One-key cleanup of temporary files"
    echo " -------------"
